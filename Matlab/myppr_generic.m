%Function for generating the Articulated Inspection Arm (CEA Robot) (page 8
%of the report).

function docNode=myppr_generic(DH)
for i=1:size(DH,1)
    M{i}=metas(DH(i,1),DH(i,2),DH(i,3),DH(i,4));
end
...
%    M015=M014*M1415;
    ...
    connector.setAttribute('x',char(M_1(1,4)));
    connector.setAttribute('y',char(M_1(2,4)));
    connector.setAttribute('z',char(M_1(3,4)));
...
%     connector.setAttribute('z',char(eval(['M0' num2str(i+1) '(3,4)'])));
    connector.setAttribute('x',char(M_2(1,4)));
    connector.setAttribute('y',char(M_2(2,4)));
    connector.setAttribute('z',char(M_2(3,4)));
    connector.setAttribute('name',char(['connector' num2str(counter_con)]));
